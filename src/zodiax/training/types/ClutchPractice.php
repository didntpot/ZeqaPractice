...
class ClutchPractice{
	...
	private ?KnockbackInfo $kbInfo;
	private int $attackTime;
	private int $hitStacks;
	private int $currentHitStacks;
	private int $hitDelaySeconds;
	private int $currentHitDelaySeconds;
	private int $blocks;
	...
	public function __construct(int $worldId, Player $player, DefaultKit $kit, TrainingArena $arena){
		...
		PracticeUtil::onChunkGenerated($this->world, $spawnPos2->getFloorX() >> 4, $spawnPos2->getFloorZ() >> 4, function() use ($player, $spawnPos1, $spawnPos2){
			...
			$this->dummyBot->getInventory()->setItemInHand(VanillaItems::AIR());
			$this->dummyBot->spawnTo($player);
			PracticeUtil::teleport($this->dummyBot, $spawnPos2, $spawnPos1);
		});
		$this->spectators = [];
		$this->chunks = [];
		$this->kbInfo = clone $kit->getKnockbackInfo();
		$this->attackTime = 0;
		$this->hitStacks = 2;
		$this->currentHitStacks = $this->hitStacks;
		$this->hitDelaySeconds = 1;
		$this->currentHitDelaySeconds = 0;
		$this->blocks = 0;
		$this->lastBlock = null;
	}
			...
			case self::STATUS_STARTING:
				if($this->currentTick % 20 === 0){
					if($this->countdownSeconds === 3){
						...
						$player->sendTitle(TextFormat::RED . "Clutch!", "", 5, 20, 5);
			...
			case self::STATUS_IN_PROGRESS:
				...
				if($this->currentTick % 10 === 0){
					if($this->currentTick % 20 === 0){
						...
						if($this->currentHitDelaySeconds > 0 && $this->currentHitStacks === $this->hitStacks){
							$this->currentHitDelaySeconds--;
						}
					}
					$player->sendPopup(PracticeCore::COLOR . "Cooldown " . TextFormat::WHITE . number_format(max($this->currentHitDelaySeconds - ($this->currentTick % 20 === 0 ? 0 : 0.5), 0), 1));
				}
				$this->dummyBot->lookAt($player->getEyePos());
				if($this->currentHitDelaySeconds === 0){
					if($this->currentHitStacks > 0){
						if($this->attackTime === 0){
							$this->dummyBot->attackEntity($player);
							$this->currentHitStacks--;
							$this->attackTime = $this->kbInfo->getSpeed();
						}else{
							$this->attackTime--;
						}
					}else{
						$this->currentHitStacks = $this->hitStacks;
						$this->currentHitDelaySeconds = $this->hitDelaySeconds;
						$player->sendPopup(PracticeCore::COLOR . "Cooldown " . TextFormat::WHITE . number_format($this->currentHitDelaySeconds, 1));
					}
				}
				break;
	...
	private function setInClutch() : void{
		/** @var TrainingArena $arena */
		$arena = ArenaManager::getArena($this->arena);
	...
	public function resetClutch() : void{
		...
		if(($session = PlayerManager::getSession($player = PlayerManager::getPlayerExact($this->playerName))) !== null){
			/** @var TrainingArena $arena */
			$arena = ArenaManager::getArena($this->arena);
			PracticeUtil::teleport($player, $arena->getP1Spawn(), $this->dummyBot->getPosition());
			$this->dummyBot->lookAt($player->getEyePos());
			$session->getKitHolder()->clearKit();
			ItemHandler::spawnClutchItems($player);
		}
	}
	...
	public function setEnded() : void{
		if($this->status !== self::STATUS_ENDED){
			...
			TrainingHandler::removeClutch($this->worldId);
		}
	}
	...
	public function setKnockbackInfo(float $horizontalKB, float $verticalKB, int $speed) : void{
		$this->kbInfo->setHorizontalKb($horizontalKB);
		$this->kbInfo->setVerticalKb($verticalKB);
		$this->kbInfo->setSpeed($speed);
		$this->attackTime = 0;
	}

	public function getHitStacks() : int{
		return $this->hitStacks;
	}

	public function setHitStacks(int $hitStacks) : void{
		$this->hitStacks = $hitStacks;
		$this->currentHitStacks = $this->hitStacks;
	}

	public function getHitDelaySeconds() : int{
		return $this->hitDelaySeconds;
	}

	public function setHitDelaySeconds(int $hitDelaySeconds) : void{
		$this->hitDelaySeconds = $hitDelaySeconds;
		$this->currentHitDelaySeconds = $this->hitDelaySeconds;
	}
