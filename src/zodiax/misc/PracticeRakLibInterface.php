...
final class PracticeRakLibInterface extends RakLibInterface{

	protected const MCPE_RAKNET_PROTOCOL_VERSION = 10;
	protected const MCPE_RAKNET_PACKET_ID = "\xfe";
	protected Server $server;
	protected Network $network;
	protected int $rakServerId;
	protected RakLibServer $rakLib;
	protected array $sessions = [];
	protected RakLibToUserThreadMessageReceiver $eventReceiver;
	protected UserToRakLibThreadMessageSender $interface;
	protected SleeperNotifier $sleeper;
	...
	public function __construct(Server $server, string $ip, int $port, bool $ipV6){
		...
		$this->interface->setPacketsPerTickLimit(PHP_INT_MAX);
	}
	...
	public function onPingMeasure(int $sessionId, int $pingMS) : void{
		if(isset($this->sessions[$sessionId])){
			PlayerManager::getSession($this->sessions[$sessionId]->getPlayer())?->updatePing($pingMS);
		}
	}
}