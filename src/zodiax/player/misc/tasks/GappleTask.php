...
class GappleTask extends AbstractRepeatingTask{
	...
	protected function onUpdate(int $tickDifference) : void{
		if($this->session->canGapple() || ($player = $this->session->getPlayer()) === null || !$player->isOnline()){
		...
		if($this->getCurrentTick() === $this->cooldown){
			$this->session->setGapple(true, $this->cooldown !== PracticeUtil::secondsToTicks(1));
			$this->getHandler()?->cancel();
		}
	}
}